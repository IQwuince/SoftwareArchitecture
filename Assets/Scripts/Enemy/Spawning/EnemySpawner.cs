...
using UnityEngine;
    ...
    public class EnemySpawner : MonoBehaviour
    {
        ...
        [SerializeField] int spawnAttempts = 6;
        ...
        readonly Dictionary<GameObject, int> originIndex = new Dictionary<GameObject, int>(); // enemy -> spawnPoints index
        float timer;
        ...
        private void Start()
        {
            ...
            {
                // optional convenience: populate entries from children if empty
        ...
        void CleanupActiveList()
        {
            ...
            // also remove missing keys in originIndex (defensive)
        ...
        void TrySpawnOne()
        {
            ...
            for (int i = 0; i < spawnPoints.Length; i++)
            {
                ...
                if (sp.maxActiveAtPoint > 0 && CountActiveAtPoint(i) >= sp.maxActiveAtPoint) continue;
            ...
            float acc = 0f;
            int chosenIndex = candidates[Random.Range(0, candidates.Count)]; // fallback
            ...
            // try to find a valid position around chosen point
            for (int attempt = 0; attempt < spawnAttempts; attempt++)
            {
                ...
                if (player != null && Vector2.Distance(player.position, pos) < chosen.minDistanceFromPlayer) continue;
                if (Physics2D.OverlapCircle(pos, chosen.spawnRadius, chosen.obstacleMask)) continue;
            ...
            // no valid position found this tick
        }

        int CountActiveAtPoint(int pointIndex)
        {
            int c = 0;
            foreach (var kv in originIndex)
            {
                if (kv.Value == pointIndex && kv.Key != null) c++;
            }
            return c;
        }

        // call this from enemy on death if you want immediate cleanup
